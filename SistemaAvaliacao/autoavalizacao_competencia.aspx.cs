...
{
    public partial class autoavalizacao_competencia : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                trocaTitulos = false; // DESATIVAR CONFIG DO BACKOFFICE

                foreach (var item in competencias)
                {
                    // Verificar se tem data de Inicio da Avaliação 
                    ...
                    if (item.IdModo == 1)
                    {
                        ...
                        var getNotaNivel1 = avaliacoesService.ObterAvaliacaoCompetenciaNota(getLinhaAvaliacao.IdNotaNivel1AutoAvaliacao);
                        var getNotaNivel2 = avaliacoesService.ObterAvaliacaoCompetenciaNota(getLinhaAvaliacao.IdNotaNivel2AutoAvaliacao);
        ...
        protected void rptCompetencias_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.AlternatingItem || e.Item.ItemType == ListItemType.Item)
            {
                ...
                bool enableAllOptions = avaliacao.DataHoraFimAutoAvaliacao != null;
        ...
        private static void CarregarAvaliacao(RepeaterItem repeaterItem, AVALIACOESCOMPETENCIAS avaliacao, COMPETENCIAS competencia)
        {
            ...
            if (avaliacao != null)
            {
                ...
                ddl2.Disabled = avaliacao.DataHoraFimAutoAvaliacao != null;
        ...
        private bool ValidarAvaliacao()
        {
            ...
            foreach (RepeaterItem item in rptCompetencias.Items)
            {
                if (item.ItemType == ListItemType.AlternatingItem || item.ItemType == ListItemType.Item)
                {
                    ...
                    {
                        notaDdl1 = getCompetencia.IdNotaPadraoNivel1 != null ? (int)getCompetencia.IdNotaPadraoNivel1 : 5;
                        notaDdl2 = getCompetencia.IdNotaPadraoNivel2 != null ? (int)getCompetencia.IdNotaPadraoNivel2 : 5;
                    }
                ...
                foreach (RepeaterItem item in rptCompetencias.Items)
                {
                    if (item.ItemType == ListItemType.AlternatingItem || item.ItemType == ListItemType.Item)
                    {
                        ...
                        if (idHiddenCompetencia != null)
                        {
                            ...
                            // Salva Alterações
                            if (avaliacaoService.AlterarAvaliacaoCompetencia(avaliacao.IdAvaliacaoCompetencia, avaliacao, true))
                            {
