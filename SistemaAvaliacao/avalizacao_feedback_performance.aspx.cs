...
{
    public partial class avalizacao_feedback_performance : System.Web.UI.Page
    {
                ...
                while (novaAbrangencia)
                {
                    ...
                    foreach (var item in performances)
                    {
                        ...
                        if (addItem)
                        {
                            ...
                            linhaPerformance.DisclaimerInput = item.InputAvaliacaoGestor ? "" : "Esta nota não requer preenchimento em feedback";
        ...
        private void CarregarAvaliacao(RepeaterItem repeaterItem)
        {
            ...
            if (avaliacao != null)
            {
                ...
                {
                    MessageBox.Show("A Avaliação Performance não está na Etapa de Feedback.", "Alteração não Permitida", TIPO.Warning, MessageBoxHandler);
                    txt1.Disabled = true;
                    ddl1.Disabled = true;
                }
            }
            ...
            {
                MessageBox.Show("A Avaliação Performance ainda não foi finalizada pelo Gestor.", "Impossível Prosseguir", TIPO.Warning, MessageBoxHandler);
                txt1.Disabled = true;
                ddl1.Disabled = true;
            }
        ...
        {
            var statusList = new NotasAvaliacaoService().ListaNotasPerformances();

            AVALIACOESPERFORMANCESNOTAS aNota = new AVALIACOESPERFORMANCESNOTAS();
            aNota.IdNota = -1;
            aNota.CodigoNota = "[Selecionar]";
            statusList.Insert(0, aNota);
        ...
        protected void btnSalvar_Click(object sender, EventArgs e)
        {
            if (SalvarAvaliacao(false))
                ...
                MessageBox.Show("Falha ao Incluir Performance do Feedback.", "Erro ao Incluir", TIPO.Error, MessageBoxHandler);
        }
                ...
                foreach (RepeaterItem item in rptPerformances.Items)
                {
                    if (item.ItemType == ListItemType.AlternatingItem || item.ItemType == ListItemType.Item)
                    {
                        ...
                        if (idHiddenPerformance != null)
                        {
                            ...
                            if (avaliacao.PosicaoAtualFluxoAvaliacao != avaliacaoService.etapaFeedback)
                            {
                                MessageBox.Show($"A Performance {idPerformance} ainda não foi finalizada pelo Gestor.", "Impossível Prosseguir", TIPO.Info, MessageBoxHandler);
                                return false;
                            }
        ...
        protected void btnIrCompetencia_Click(object sender, EventArgs e)
        {
            ...
            Response.Redirect("~/avalizacao_feedback_competencia.aspx");
