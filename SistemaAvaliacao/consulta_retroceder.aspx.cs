...
{
    public partial class consulta_retroceder : System.Web.UI.Page
    {
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            if (ddlFase.SelectedIndex > 0)
                avaliacoes = avaliacoes.Where(x => x.PosicaoAtualFluxoAvaliacao == ddlFase.Items[ddlFase.SelectedIndex].Value).ToList();
            ...
            foreach (var avaliacao in avaliacoes)
            {
                ...
                umaAvaliacao.Gestor = associadosService.ObterAssociado((int)avaliacao.idGestor);// associadosService.ObterGestor(avaliacao.idProjeto, avaliacao.idAssociado);
                ...
                switch (avaliacao.PosicaoAtualFluxoAvaliacao)
                {
                    ...
                    case etapaAutoAvaliacao:
                        umaAvaliacao.Fase = "Em Auto avaliação e às Cegas";                        
                        break;
        ...
        protected void btnRetroceder_Click(object sender, EventArgs e)
        {
            ...
            ckb_incluirAutoAvaliacao.Visible = false;
            ckb_incluirAsCegas.Visible = false;
            pnlRetroceder.Visible = false;
            ...
            if (avaliacao != null)
            {
                ...
                ckb_incluirAutoAvaliacao.Visible = autoAvaliacaoFinalizada;
                ckb_incluirAsCegas.Visible = avaliacaoAsCegasFinalizada;

                pnlRetroceder.Visible = autoAvaliacaoFinalizada || avaliacaoAsCegasFinalizada;

                if (!autoAvaliacaoFinalizada && !avaliacaoAsCegasFinalizada)
                {
                    MessageBox.Show("Não é possível retroceder a avaliação. Auto avaliação e Avaliação às Cegas não concluídas", "Alerta", TIPO.Error, MessageBoxHandler);
                    btnEfetuarRetrocesso.Visible = false;
                }
                ...
                else
                {
                    ...
                    if (idPeriodo == periodoVigente.IdPeriodo || 1 == 1)
                    {
                        if (periodoVigente.DataFim >= DateTime.Today || 1 == 1)
                        {
                            ...
                            ddlFase.Items[ddlFase.SelectedIndex].Value = avaliacao.PosicaoAtualFluxoAvaliacao;
        ...
        protected void btnEfetuarRetrocesso_Click(object sender, EventArgs e)
        {
            ...
            if (avaliacao != null)
            {
                var etapasPermitidas = new[] {
                    etapaAvaliacaoAsCegas,
                    etapaAutoAvaliacao,
                    etapaNaoIniciada,
                    etapaEmParalelo
                };

                if (etapasPermitidas.Contains(avaliacao.PosicaoAtualFluxoAvaliacao) && ddlFase.Items[ddlFase.SelectedIndex].Value != etapaEmParalelo)
                {
                    // avançar a avaliação somente se todas as questões estiverem respondidas
                    // verificar cada etapa para avançar a avaliação
                    ...
                    //Mentor
                    var competenciasMentorEstaoRespondidas = competencias.All(c =>
                                                                    c.IdNotaNivel1AvaliacaoMentor != null &&
                                                                    c.IdNotaNivel2AvaliacaoMentor != null
                                                                    );

                    var performancesMentorEstaoRespondidas = performances.All(c =>
                                                                    c.IdNotaNivel1AvaliacaoMentor != null
                                                                    );
                    //Feedback
                    ...
                    // Verifica cada etapa que a avaliação pode estar
                    switch (ddlFase.Items[ddlFase.SelectedIndex].Value)
                    {
                        case etapaAvaliacaoGestor:
                            if (!competenciasAutoCegasEstaoRespondidas && !performancesEstaoAutoCegasRespondidas)
                            {
                                MessageBox.Show("Não é possível avançar a etapa da avaliação. Auto avaliação e/ou Avaliação às Cegas não concluídas", "Alerta", TIPO.Error, MessageBoxHandler);
                                return;
                            }
                            else
                            {
                                foreach (var item in competencias)
                                {
                                    ...
                                    item.DataHoraFimAutoAvaliacao = item.DataHoraFimAutoAvaliacao == null ? DateTime.Now : item.DataHoraFimAutoAvaliacao;
                                    item.DataHoraFimAvaliacaoCegas = item.DataHoraFimAvaliacaoCegas == null ? DateTime.Now : item.DataHoraFimAvaliacaoCegas;
                                    item.PosicaoAtualFluxoAvaliacao = ddlFase.Items[ddlFase.SelectedIndex].Value;
                                    dm.SaveChanges();
                                }
                                ...
                                avaliacao.PosicaoAtualFluxoAvaliacao = ddlFase.Items[ddlFase.SelectedIndex].Value;
                                dm.SaveChanges();
                        ...
                        case etapaFeedback:
                            ...
                            {
                                MessageBox.Show("Não é possível avançar a etapa da avaliação. Avaliação do Gestor não concluída", "Alerta", TIPO.Error, MessageBoxHandler);
                                return;
                            }
                            else
                            {
                        ...
                        case etapaAvaliacaoMentor:
                            ...
                            {
                                MessageBox.Show("Não é possível avançar a etapa da avaliação. Feedback não concluído", "Alerta", TIPO.Error, MessageBoxHandler);
                                return;
                            }
                            else
                            {
                ...
                else if(etapasPermitidas.Contains(avaliacao.PosicaoAtualFluxoAvaliacao) && ddlFase.Items[ddlFase.SelectedIndex].Value == etapaEmParalelo)
                {
                    var competencias = dm.AVALIACOESCOMPETENCIAS.Where(x => x.IdAssociado == avaliacao.idAssociado && x.IdProjeto == avaliacao.idProjeto && x.IdPeriodo == avaliacao.idPeriodo).ToList();
                    ...
                    {
                        item.IdAvaliacaoStatus = 2;
                        // Se a avaliação estivar em etapa Em paralelo e a opção de incluir auto avaliação ou avaliação às cegas não estiver marcada, limpa as datas de fim
                        
                        item.DataHoraFimAutoAvaliacao = ckb_incluirAutoAvaliacao.Checked ? null : item.DataHoraFimAutoAvaliacao;
                        item.DataHoraFimAvaliacaoCegas = ckb_incluirAsCegas.Checked ? null : item.DataHoraFimAvaliacaoCegas;
                        item.PosicaoAtualFluxoAvaliacao = etapaEmParalelo;
                        dm.SaveChanges();
                    }
                ...
                else if (avaliacao.PosicaoAtualFluxoAvaliacao != ddlFase.Items[ddlFase.SelectedIndex].Value)
                {
                    if (ddlFase.Items[ddlFase.SelectedIndex].Value == etapaEmParalelo)
                    {
                        // Se a fase selecionada for Em Paralelo, limpa as datas de fim de auto avaliação e avaliação às cegas
                        var competencias = dm.AVALIACOESCOMPETENCIAS.Where(x => x.IdAssociado == avaliacao.idAssociado && x.IdProjeto == avaliacao.idProjeto && x.IdPeriodo == avaliacao.idPeriodo).ToList();
                        foreach (var item in competencias)
                        {
                            item.IdAvaliacaoStatus = 2;
