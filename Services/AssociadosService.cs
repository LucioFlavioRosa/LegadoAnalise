...
namespace Peers.Moderno.Services
{
    public class AssociadosService : IAssociadosService
    {
        ...
        {
            return await _context.Associados.Include(a => a.CARGOS).Include(a => a.PERFIS).Include(a => a.EMPRESAS).Include(a => a.ASSOCIADOS2).ToListAsync();
        }

        public async Task<ASSOCIADOS> ObterAssociadoAsync(int id)
        {
            return await _context.Associados.Include(a => a.CARGOS).Include(a => a.PERFIS).Include(a => a.EMPRESAS).Include(a => a.ASSOCIADOS2).FirstOrDefaultAsync(a => a.IdAssociado == id);
        }

        public async Task<ASSOCIADOS> ObterAssociadoPorEmailAsync(string email)
        {
            return await _context.Associados.FirstOrDefaultAsync(a => a.Email == email);
        }
