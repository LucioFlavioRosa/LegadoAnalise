...
namespace Peers.Moderno.Services
{
    public class CargosService : ICargosService
    {
        ...
        {
            return await _context.Cargos.Where(c => !ativo || c.ATV == 1).ToListAsync();
        }

        public async Task<List<PROMOCOES>> ObterTodasPromocoesAssociadoAsync(int idAssociado)
        {
            return await _context.Promocoes.Where(p => p.idAssociado == idAssociado).Include(p => p.CARGOS).Include(p => p.CARGOS1).ToListAsync();
        }
        ...
        {
            return await _context.Promocoes.Include(p => p.ASSOCIADOS).Include(p => p.CARGOS).Include(p => p.CARGOS1).ToListAsync();
        }

        public async Task<PROMOCOES> ObterPromocaoAsync(int idAssociado, int idCargoAnterior, int idCargoNovo)
        {
            return await _context.Promocoes.FirstOrDefaultAsync(p => p.idAssociado == idAssociado && p.idCargoAnterior == idCargoAnterior && p.idCargoNovo == idCargoNovo);
        }
